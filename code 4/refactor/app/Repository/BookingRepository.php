...
//            $jobs['total'] = $totalJobs;
            return ['emergencyJobs' => $emergencyJobs, 'noramlJobs' => $normalJobs, 'jobs' => $jobs, 'cuser' => $currentUser, 'usertype' => $userType, 'numpages' => $numPages, 'pagenum' => $pageNum];
        }
    }
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
     ...
     * @param $is_need_delay
     */
    public function sendPushNotificationToSpecificUsers($users, $jobId, $data, $msgText, $is_need_delay)
    {
        ...
        $logger->pushHandler(new StreamHandler(storage_path('logs/push/laravel-' . date('Y-m-d') . '.log'), Logger::DEBUG));
        $logger->pushHandler(new FirePHPHandler());
        $logger->addInfo('Push send for job ' . $jobId, [$users, $data, $msgText, $is_need_delay]);
        if (env('APP_ENV') == 'prod') {
...
//        $jobs = TeHelper::convertJobIdsInObjs($jobIds);
    ...
    public function updateJob($id, $data, $currentUser)
    {
        ...
        $this->logger->addInfo('USER #' . $currentUser->id . '(' . $currentUser->name . ')' . ' has been updated booking <a class="openjob" href="/admin/jobs/' . $id . '">#' . $id . '</a> with data:  ', $logData);
...
//                Event::fire(new JobWasCanceled($job));
    ...
    public function getPotentialJobs($currentUser)
    {
        ...
        $translatorLevel = $currentUser_meta->translator_level;
